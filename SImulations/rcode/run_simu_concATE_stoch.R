...
# #All_WE stoch
# aipw_all_stoch(formula_m0_WE,formula_m1_WE,formula_me,formula_vk,data,nonpara=F)
# 
# 
# 
# point_oc_w <- point_oc_we <- point_all_w <- point_all_w_s <- point_all_we <- true <- NULL
# se_oc_w <- se_oc_we <- se_all_w <- se_all_we <- se_all_w_s <- NULL
# itera <- 1000
# for(i in 1:itera){
#   if( (i == 1) | (i %% 250)==0){print(paste("Iteration,", i, "out of", itera)) }
# data <- data_genera(n,tr_effectA,beta_W_Y,beta_W_E,beta_E_Y,
...
# 
#   all_w  <- aipw_all(formula_m0_WE,formula_m1_WE,formula_me,data,nonpara=F)
#   
#   all_w_s <- aipw_all_stoch(formula_m0_WE,formula_m1_WE,formula_me,formula_vk,data,nonpara=F)
# 
#   point_oc_w[i]   <- oc_w$mu
#   point_oc_we[i]  <- oc_we$mu
#   point_all_w[i]  <- all_w$mu
#   point_all_w_s[i] <- all_w_s$mu
# 
#   se_oc_w[i]   <- oc_w$se_if
#   se_oc_we[i]  <- oc_we$se_if
#   se_all_w[i]  <- all_w$se_if
#   se_all_w_s[i]  <- all_w_s$se_if
#   true[i] <- mean(data_Vk$Y1) - mean(data_Vk$Y0)
# 
# }
# 
...
# 
# mean(point_all_w);sd(point_all_w);mean(se_all_w)
# mean(point_all_w_s);sd(point_all_w_s);mean(se_all_w_s)
# 
# sd(point_oc_w)/sd(point_all_w)
# sd(point_oc_we)/sd(point_all_w)
# sd(point_all_w)/sd(point_all_w_s)
...
aipw_point_oc_w <- aipw_point_oc_we <- aipw_point_all <- aipw_point_all_S <- NULL
gcomp_se_oc_w <- gcomp_se_oc_we <- gcomp_se_all_w <- gcomp_se_all_we <- NULL
aipw_se_oc_w <- aipw_se_oc_we <- aipw_se_all <- aipw_se_all_S <- NULL
...
mse_gcomp_point_oc_w <- mse_gcomp_point_oc_we <- mse_gcomp_point_all_we <- 
  ...
  mse_ipw_point_oc_w <- mse_ipw_point_oc_we <- mse_aipw_point_all_we_S <- NULL
...
bias_gcomp_point_oc_w <- bias_gcomp_point_oc_we <- bias_gcomp_point_all_we <- 
  ...
  bias_ipw_point_oc_w <- bias_ipw_point_oc_we <- bias_aipw_point_all_we_S <- NULL
...
se_aipw_point_oc_w_sv <- se_aipw_point_oc_we_sv <- se_aipw_point_all_we_sv <- 
  se_aipw_point_oc_w_if <- se_aipw_point_oc_we_if <- se_aipw_point_all_we_if <- 
  se_aipw_point_all_we_if_S <- se_aipw_point_all_we_sv_S <- NULL

ratio_gcomp_oc_w_vs_all <- ratio_gcomp_oc_we_vs_all <- ratio_aipw_oc_w_vs_all <- 
  ratio_aipw_oc_we_vs_all <- ratio_aipw_oc_we_vs_all_S <-  NULL
...
M_mse_gcomp_point_oc_w <- M_mse_gcomp_point_oc_we <- M_mse_gcomp_point_all_we <-
  ...
  M_mse_ipw_point_oc_w <- M_mse_ipw_point_oc_we <- M_mse_aipw_point_all_we_S <- NULL
...
coverage_gcompOR_w <- coverage_gcompOR_we <- coverage_gcompall_w <- 
  ...
  coverage_aipw_point_all_w <- coverage_ipw_point_oc_w <- coverage_ipw_point_oc_we <- 
  coverage_aipw_point_all_w_S <- NULL
...
M_coverage_gcompOR_w <- M_coverage_gcompOR_we <- M_coverage_gcomp_all_w <-
  ...
  M_coverage_aipw_all <- M_coverage_ipw_oc_w <- M_coverage_ipw_oc_we <- 
  M_coverage_aipw_all_S <- NULL
...
for(th in seq_th){
  ...
  k <- k+1 
    for(i in 1:itera){
          ...
          aipw_all_w_s <- aipw_all_stoch(formula_m0_WE,formula_m1_WE,formula_me,formula_vk,data,nonpara=F)
          ...
          aipw_point_all_S[i]  <- aipw_all_w_s$mu
          ...
          aipw_se_all_S[i]  <- aipw_all_w_s$se_if
          true[i] <- mean(data_Vk$Y1) - mean(data_Vk$Y0)
          ...
          mse_aipw_point_all_we_S[i]  <- (aipw_point_all_S[i] - true[i])^2
          ...
          coverage_aipw_point_all_w_S[i] <- coverage(aipw_point_all_S[i],
                                                   aipw_se_all_S[i],true[i])
    ...
    #bias
    bias_gcomp_point_oc_w[k] <- (mean(gcomp_point_oc_w) - mean(true))
    bias_gcomp_point_oc_we[k] <- (mean(gcomp_point_oc_we) - mean(true))
    bias_gcomp_point_all_we[k] <- (mean(gcomp_point_all_we) - mean(true))
    
    bias_ipw_point_oc_w[k] <- (mean(IPW_point_oc_w) - mean(true))
    bias_ipw_point_oc_we[k] <- (mean(IPW_point_oc_we) - mean(true))
    
    bias_aipw_point_oc_w[k] <- (mean(aipw_point_oc_w) - mean(true))
    bias_aipw_point_oc_we[k] <- (mean(aipw_point_oc_we) - mean(true))
    bias_aipw_point_all_we[k] <- (mean(aipw_point_all) - mean(true))
    bias_aipw_point_all_we_S[k] <- (mean(aipw_point_all_S) - mean(true))
    
    #Standard error
    se_gcomp_point_oc_w_sv[k] <- sd(gcomp_point_oc_w)
    se_gcomp_point_oc_we_sv[k] <- sd(gcomp_point_oc_we)
    se_gcomp_point_all_we_sv[k] <- sd(gcomp_point_all_we)
    
    se_gcomp_point_oc_w_if[k] <- mean(se_oc_w)
    se_gcomp_point_oc_we_if[k] <- mean(se_oc_we)
    se_gcomp_point_all_we_if[k] <- mean(se_all_we)
    
    
    se_ipw_point_oc_w_sv[k] <- sd(IPW_point_oc_w)
    se_ipw_point_oc_we_sv[k] <- sd(IPW_point_oc_we)
    se_ipw_point_oc_w_if[k] <- mean(ipw_se_oc_w)
    se_ipw_point_oc_we_if[k] <- mean(ipw_se_oc_we)
    
    
    se_aipw_point_oc_w_sv[k] <- sd(aipw_point_oc_w)
    se_aipw_point_oc_we_sv[k] <- sd(aipw_point_oc_we)
    se_aipw_point_all_we_sv[k] <- sd(aipw_point_all)
    se_aipw_point_all_we_sv_S[k] <- sd(aipw_point_all_S)
    
    se_aipw_point_oc_w_if[k] <- mean(aipw_se_oc_w)
    se_aipw_point_oc_we_if[k] <- mean(aipw_se_oc_we)
    se_aipw_point_all_we_if[k] <- mean(aipw_se_all)
    se_aipw_point_all_we_if_S[k] <- mean(aipw_se_all_S)
    

    
    #efficiency gains:
    # sd(gcomp_point_oc_w)/sd(gcomp_point_oc_we)
    ratio_gcomp_oc_w_vs_all[k] <- sd(gcomp_point_oc_w)/sd(gcomp_point_all_w)
    ratio_gcomp_oc_we_vs_all[k] <- sd(gcomp_point_oc_we)/sd(gcomp_point_all_we)
    
    # sd(aipw_point_oc_w)/sd(aipw_point_oc_we)
    ratio_aipw_oc_w_vs_all[k] <- sd(aipw_point_oc_w)/sd(aipw_point_all)
    ratio_aipw_oc_we_vs_all[k] <- sd(aipw_point_oc_we)/sd(aipw_point_all)
    ratio_aipw_oc_we_vs_all_S[k] <- sd(aipw_point_oc_we)/sd(aipw_point_all_S)
    
    M_mse_gcomp_point_oc_w[k] <- mean(mse_gcomp_point_oc_w)
    M_mse_gcomp_point_oc_we[k] <- mean(mse_gcomp_point_oc_we)
    M_mse_gcomp_point_all_we[k] <- mean(mse_gcomp_point_all_we)
    
    M_mse_aipw_point_oc_w[k] <- mean(mse_aipw_point_oc_w)
    M_mse_aipw_point_oc_we[k] <- mean(mse_aipw_point_oc_we)
    M_mse_aipw_point_all_we[k] <- mean(mse_aipw_point_all_we)
    M_mse_aipw_point_all_we_S[k] <- mean(mse_aipw_point_all_we_S)
    
    
    M_mse_ipw_point_oc_w[k] <- mean(mse_ipw_point_oc_w)
    M_mse_ipw_point_oc_we[k] <- mean(mse_ipw_point_oc_we)
    
  
    M_coverage_gcompOR_w[k] <- mean(coverage_gcompOR_w)
    M_coverage_gcompOR_we[k] <- mean(coverage_gcompOR_we)
    M_coverage_gcomp_all_w[k] <- mean(coverage_gcompall_w)
    M_coverage_gcomall_we[k] <- mean(coverage_gcompall_we)

    M_coverage_ipw_oc_w[k] <- mean(coverage_ipw_point_oc_w)
    M_coverage_ipw_oc_we[k] <- mean(coverage_ipw_point_oc_we)
        
    M_coverage_aipw_oc_w[k] <- mean(coverage_aipw_point_oc_w)
    M_coverage_aipw_oc_we[k] <- mean(coverage_aipw_point_oc_we)
    M_coverage_aipw_all[k] <- mean(coverage_aipw_point_all_w)
    M_coverage_aipw_all_S[k] <- mean(coverage_aipw_point_all_w_S)    
...
method <- c(rep("Gcomp-OC-W",length(seq_th)),
            ...
            rep("AIPW-All-Stoch",length(seq_th)))

threshold <- rep(seq_th,9)
...
#save.image("/Users/santam13/Documents/NYU/Non concurrent/Ident-Estima-Parametric/rcode/non-concurrent/data_simu/Correct-Outcome Correct-Treatment_11.15.24 - Stochastic.RData")
save.image("/Users/santam13/Documents/NYU/Non concurrent/Ident-Estima-Parametric/rcode/non-concurrent/data_simu/Miss-Outcome Correct-Treatment_11.15.24 - Stochastic.RData")
#save.image("/Users/santam13/Documents/NYU/Non concurrent/rcode/non-concurrent/data_simu/Correct-Outcome Miss-Treatment_04.25.24.RData")
#save.image("/Users/santam13/Documents/NYU/Non concurrent/rcode/non-concurrent/data_simu/Miss-Outcome Miss-Treatment_04.25.24.RData")
